  ...
  try {
    ...
    data.forEach(item => {
      ...
      map[item.category_id].items.push({
        name: item.name, desc: item.description, price: item.price, min: item.min_order, image: item.image_url || null,
      })
    })
  ...
  return (
    <div className="bg-white rounded-xl border border-brown/8 p-4 flex flex-col gap-3 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
      ...
      <div className="flex items-center justify-between pt-2 border-t border-brown/6">
        <span className="text-terra font-bold text-sm">RM {parseFloat(item.price).toFixed(2)} <span className="text-muted font-normal text-xs">/ biji</span></span>
        {item.min > 1 && (
    ...
    <div className="min-h-screen bg-white pt-[70px]">
      ...
      <div className="bg-cream2 py-14 text-center px-6">
        ...
        <div className="flex gap-4 justify-center flex-wrap">
          <Link href="/reservations" className="bg-forest text-cream px-8 py-3.5 rounded-lg font-semibold hover:bg-brown transition-colors">
